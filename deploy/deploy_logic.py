...
class ReleaseBuilder:
    ...
    def _build_exe(self):
        ...
        command = [
            ...
            "holidays",
            "--icon",
            "assets\\production_counter_icon.ico",
            "--add-data",
            "assets\\production_counter_icon.ico;assets",
            "--add-data",
            "project\\config\\profile_config.csv;project\\config",
            "--add-data",
            "project\\config\\machine_config.csv;project\\config",
            "--add-data",
            "project\\config\\double_sided_machines.json;project\\config",
            "--add-data",
            "project\\core\\logic\\templates\\report_template.docx;project\\core\\logic\\templates",
            "--add-data",
            "project\\data\\help_sections.json;project\\data",
            "--add-data",
            "project\\data\\help_images;project\\data\\help_images",
            "app.py",
        ]

        # Uruchamiamy proces w katalogu głównym projektu (cwd=self.base_dir)
        result = subprocess.run(command, cwd=str(self.base_dir), capture_output=True, text=True)
    ...
    def _pack_to_zip(self) -> Path:
        ...
        shutil.make_archive(
            base_name=str(self.zip_temp_path), format="zip", root_dir=str(self.dist_folder)
        )

        final_zip_path = Path(f"{self.zip_temp_path}.zip")
        self.log(
            f"Archiwum gotowe: {final_zip_path.name} ({final_zip_path.stat().st_size // 1024} KB)"
        )
