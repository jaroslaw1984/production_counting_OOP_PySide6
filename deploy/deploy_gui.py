...
class DeployApp(ctk.CTk):
    ...
    def _create_widgets(self):
        ...
        self.version_label = ctk.CTkLabel(
            self.version_frame, text="Nowa wersja:", font=ctk.CTkFont(family="Segoe UI", size=14)
        )
        self.version_label.pack(side="left", padx=(0, 10))

        self.version_entry = ctk.CTkEntry(
            self.version_frame, width=150, placeholder_text="Wpisz wersję..."
        )
        ...
        self.notes_textbox = ctk.CTkTextbox(self.notes_frame, height=80, activate_scrollbars=True)
        ...
        self.console_label = ctk.CTkLabel(
            self, text="Status operacji:", font=ctk.CTkFont(family="Segoe UI", size=14)
        )
    ...
    def start_deployment(self):
        ...
        def on_process_done(success: bool):
            ...
            self.after(
                0, lambda: self.deploy_btn.configure(state="normal", text="Zbuduj i opublikuj")
            )
            if success:
