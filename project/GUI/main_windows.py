...
from project.config.paths import (
    CONFIG_PATH,
    DOUBLE_SIDED_MACHINES_PATH,
    LATEST_JSON_PATH,
    MACHINE_CONFIG_PATH,
)
...
from project.GUI.popups import (
    AboutPopup,
    CalcModePopup,
    HelpWindow,
    MachineSelectPopup,
...
class MainWindow:
    def __init__(self, state: AppState):
        ...
        self.welcome_inner: ctk.CTkFrame | None = None
        self.placeholder_desc: ctk.CTkLabel | None = None
        self.placeholder_logo: ctk.CTkLabel | None = None
        self.placeholder_title: ctk.CTkLabel | None = None
        self.placeholder_ver: ctk.CTkLabel | None = None
        self.table_frame: ctk.CTkScrollableFrame | None = None
    ...
    def _build_left_panel(self):
        ...
        for row, (label, handler) in enumerate(top_buttons):
            btn = ctk.CTkButton(self.top_frame, text=label, command=handler, font=self.default_font)
        ...
        for row, (label, handler) in enumerate(bottom_buttons, start=1):
            btn = ctk.CTkButton(
                self.down_frame, text=label, command=handler, font=self.default_font
            )
            btn.grid(row=row, column=0, sticky="ew", padx=10, pady=10)
    ...
    def _build_right_panel(self):
        ...
        self.print_btn = ctk.CTkButton(
            self.action_frame,
            text="Drukuj raport",
            command=self.handle_print_report,
            width=140,
            height=35,
        )
        ...
        self.export_foil_btn = ctk.CTkButton(
            self.action_frame,
            text="Wyślij zapotrzebowanie na folię",
            command=self.export_foil_report,
            width=140,
            height=35,
        )
    ...
    def _build_welcome_screen(self):
        ...
        self.welcome_inner = ctk.CTkFrame(
            self.welcome_frame, fg_color=("#e5e5e5", "#1e1e1e"), corner_radius=15
        )
    ...
    def render_db_report_cards(self, report_text: str):
        ...
        machines_data = []
        # Teraz current przechowuje instancję Twojej własnej klasy (lub None)
        current: MachineReport | None = None
        ...
        for line in report_text.splitlines():
            ...
            if line.startswith("===") and line.endswith("==="):
                ...
                # Zamiast słownika, tworzysz obiekt
                current = MachineReport(name=line.replace("=", "").strip())
                continue

            if current is not None:
                if line.startswith("Przewidywana produkcja do:"):
                    # Zamiast current["end"], używasz current.end
                    ...
                    # Używasz kropki zamiast nawiasów kwadratowych, .append() zadziała bez błędu!
        ...
        for m_data in machines_data:
            ...
            if m_data.get("end"):
                ...
                end_title = ctk.CTkLabel(
                    end_frame,
                    text="Przewidywana produkcja do:",
                    font=ctk.CTkFont(size=13),
                    text_color=("#155724", "#85c894"),
                )
                end_title.pack(side="left", padx=10, pady=8)
    ...
    def ask_calc_mode_popup(
        self, workplace: str, default_speed: float, default_pieces_per_shift: int
    ) -> dict | None:
        ...
        popup = CalcModePopup(
            self.root, workplace, default_speed, default_pieces_per_shift, on_confirm=on_confirm
        )
        self.root.wait_window(popup)
        return result["value"]
