...
from project.config.count_per_loader import (
    add_workplace,
    delete_workplace,
    fetch_workplace_config,
    update_workplace_full,
)
...
class ConfigWindow(ctk.CTkToplevel):
    ...
    def _build_ds_machines_tab(self):
        ...
        self.ds_entry = ctk.CTkEntry(
            self.ds_form_frame, placeholder_text="Nazwa maszyny (np. Maszyna 11)"
        )
        self.ds_entry.pack(side="left", fill="x", expand=True, padx=(10, 10), pady=10)

        self.ds_add_btn = ctk.CTkButton(
            self.ds_form_frame, text="Dodaj maszynę", command=self._add_ds_machine
        )
    ...
    def _add_ds_machine(self):
        ...
        if success:
            ...
            messagebox.showerror(
                "Błąd", "Nie udało się zapisać maszyny lub taka maszyna już istnieje."
            )
    ...
    def _build_machines_tab(self):
        ...
        self.mac_wp_entry = ctk.CTkEntry(
            self.mac_form_frame, placeholder_text="Stanowisko (np. WLO-U001)", width=180
        )
        self.mac_wp_entry.pack(side="left", padx=(10, 5), pady=10)

        self.mac_speed_entry = ctk.CTkEntry(
            self.mac_form_frame, placeholder_text="Prędkość (m/min)", width=120
        )
        self.mac_speed_entry.pack(side="left", padx=5, pady=10)

        self.mac_count_entry = ctk.CTkEntry(
            self.mac_form_frame, placeholder_text="Sztuki/zmiana", width=120
        )
        self.mac_count_entry.pack(side="left", padx=5, pady=10)

        self.mac_save_btn = ctk.CTkButton(
            self.mac_form_frame, text="Zapisz", command=self._save_machine, width=100
        )
    ...
    def _delete_machine(self, workplace: str):
        if messagebox.askyesno(
            "Potwierdzenie", f"Czy na pewno chcesz usunąć stanowisko {workplace} z bazy?"
        ):
    ...
    def _build_profiles_tab(self):
        ...
        self.prof_search_entry = ctk.CTkEntry(
            self.prof_search_frame, placeholder_text="Szukaj profilu..."
        )
        self.prof_search_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))

        # --- ZMIANY: Przyciski Szukaj i Reset ---
        self.prof_search_btn = ctk.CTkButton(
            self.prof_search_frame, text="Szukaj", command=self._filter_profiles, width=80
        )
        self.prof_search_btn.pack(side="left", padx=(0, 5))

        self.prof_reset_btn = ctk.CTkButton(
            self.prof_search_frame,
            text="Reset",
            width=80,
            fg_color="#555555",
            hover_color="#333333",
        ...
        self.prof_name_entry = ctk.CTkEntry(
            self.prof_form_frame, placeholder_text="Profil (np. GP8280)", width=160
        )
        self.prof_name_entry.pack(side="left", padx=(10, 5), pady=10)

        self.prof_side_entry = ctk.CTkEntry(
            self.prof_form_frame, placeholder_text="Strona (np. 0022)", width=120
        )
        self.prof_side_entry.pack(side="left", padx=5, pady=10)

        self.prof_time_entry = ctk.CTkEntry(
            self.prof_form_frame, placeholder_text="Czas (min)", width=100
        )
        self.prof_time_entry.pack(side="left", padx=5, pady=10)

        self.prof_save_btn = ctk.CTkButton(
            self.prof_form_frame, text="Zapisz", command=self._save_profile, width=100
        )
    ...
    def _refresh_profiles_list(self, profiles_to_show: list):
        ...
        for p in profiles_to_show:
            ...
            info_text = (
                f"Profil: {p['profile']}  |  Strona: {p['side']}  |  Czas: {p['setting_time']} min"
            )
            lbl = ctk.CTkLabel(row_frame, text=info_text, anchor="w")
            lbl.pack(side="left", padx=10)
    ...
    def _save_profile(self):
        ...
        if success:
            ...
            messagebox.showerror(
                "Błąd", "Nie udało się zapisać profilu w bazie danych SQL. Sprawdź połączenie."
            )

    def _delete_profile(self, profile: str, side: str):
        if messagebox.askyesno(
            "Potwierdzenie", f"Czy na pewno usunąć profil {profile} (strona {side})?"
        ):
