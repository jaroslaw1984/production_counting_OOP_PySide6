...
class ConfigDataManager:
    def __init__(
        self,
        ds_machines_path: str | Path,
        machines_csv_path: str | Path,
        profiles_csv_path: str | Path,
    ):
    ...
    def get_ds_machines(self) -> list[str]:
        if not self.ds_machines_path.exists():
            return []
        try:
            with open(self.ds_machines_path, encoding="utf-8") as f:
                return json.load(f)
    ...
    def _write_csv(
        self, file_path: Path, fieldnames: list[str], data: list[dict[str, str]]
    ) -> bool:
        try:
    ...
    def save_machine(self, workplace: str, speed: str, count: str) -> bool:
        ...
        if not updated:
            machines.append(
                {"workplace": workplace, "speed_m_per_min": speed, "count_by_shift": count}
            )
    ...
    def delete_profile(self, profile: str, side: str) -> bool:
        ...
        if not db_success:
            print(
                f"UWAGA: Nie usunięto z bazy (geometria {profile_clean} może nie istnieć lub brak połączenia)."
            )
