...
from project.config.aliases import (
    ARTICLE_ALIASES,
    GOOD_PRODUKTION_ALIASES,
    GRUNDPROFIL_ALIASES,
    ORDER_ALIASES,
)
from project.config.count_per_loader import fetch_profiles_config, update_count_by_shift
from project.config.db_loader import (
    fetch_available_machines,
    fetch_orders_for_machines,
    fetch_sap_basic_profiles,
    normalize_db_df,
)
from project.config.paths import MACHINE_CONFIG_PATH
from project.config.workplace_config_provider import (
    merge_db_and_csv_config,
    normalize_profiles_db_df,
)
...
class MainController:
        ...
        try:
            ...
            if source == "db+csv":
                self.view.show_warning(
                    "Konfiguracja", f"Konfiguracja: DB + CSV (brak w DB: {len(missing)})"
                )

            # --- Sukces połączenia co spowoduje wyświetlenie wyniku listy maszyn w popup
            self.view.show_machine_select_popup(
                machines, self.state.machine_cfg, self.on_machines_selected
            )

        except Exception as e:
            # --- Błąd (np. brak VPN / sterownika) -> polecenie dla widoku, aby wyświetlił błąd ---
            error_msg = (
                "Brak sterownika ODBC / brak dostępu do sieci firmowej.\n\n"
    ...
    def on_machines_selected(
        ...
        if should_save_config and changes:
            ...
            df_mc_df.to_csv(
                MACHINE_CONFIG_PATH, sep=";", index=False, encoding="utf-8"
            )  # Zapis do CSV
            ...
            if self.state.machine_cfg_source in ("db", "db+csv"):
                try:
                    ...
                    self.view.show_warning(
                        "Uwaga", f"Zapisano do pliku CSV, ale błąd zapisu DB:\n{e}"
                    )

            # --- odświeżenie stanu ---
            df_cfg, source, _missing = merge_db_and_csv_config(sync_missing_to_db=False)
            self.state.machine_cfg = df_cfg
            self.state.machine_cfg_source = source
        ...
        if kind == "sap" and self.state.last_report_data:
            try:
                ...
                self.view.show_error(
                    "Błąd edycji", f"Nie udało się wygenerować/otworzyć DOCX:\n{e}"
                )
                traceback.print_exc()
        else:
    ...
    def handle_export_foil_report(self):
        ...
        if not hasattr(self.state, "last_report_data") or not self.state.last_report_data:
            self.view.show_warning(
                "Brak danych", "Najpierw wygeneruj raport SAP przed wysłaniem folii."
            )
            return
        ...
        if not linia:
            self.view.show_warning(
                "Brak danych", "Brak informacji o linii produkcyjnej w raporcie."
            )
            return
        ...
        if is_correct:
            ...
            temp_popup_handler = ReportParamsPopup(
                self.view.root, machines=[], on_confirm=lambda: None
            )
    ...
    # --- Generowanie raportu po wybraniu parametrów przez użytkownika ---
    def on_report_params_selected(self, params: dict):
        ...
        if self.state.smart_plan_df is not None:
            try:
                ...
                print("Uwaga: Brak zlecenia w Smart Planie. Wyłączam Smart Matching.")
                self.view.show_warning(
        ...
        try:
            ...
            if df_sap is None or df_sap.empty:
                raise ValueError(
                    f"Nie znaleziono danych SAP dla linii: {linia_value} w dniu: {day_value}"
                )
        except Exception as e:
        ...
        try:
            ...
            # ---------------------------------------
            # DODANE: Pobranie dodatkowych danych z wyniku dopasowania
            # (np. sap_user) i zapis do stanu.
            # blocks = wynik["blocks"]
            # lines = wynik["lines"]
        ...
        if missing_articles:
            ...
            self.view.show_warning(
                ...
                "Wykryto konflikt: na liście artykułów brakuje strony wewnętrznej "
                f"0022.\n\nSzczegóły:\n{preview}",
            )
    ...
    def _load_hydra_file(self, file_path: str):
            ...
            try:
                ...
                # 1. Tworzymy lokalną zmienną 'temp_df' zamiast od razu pisać do 'self.df_raw'
                if ext in (".xlsx", ".xls"):
                    temp_df = pd.read_excel(file_path, engine="openpyxl", header=1)
                elif ext == ".csv":
                    temp_df = pd.read_csv(
                        file_path, encoding="utf-8-sig", sep=",", low_memory=False
                    )
                else:
                ...
                # 2. Operujemy cały czas na zmiennej lokalnej (linter wie, że to pd.DataFrame)
                cleaned_columns = pd.Index(
                    [
                        " ".join(str(column).replace("\xa0", " ").strip().split())
                ...
                # 3. Zapisujemy w pełni gotową tabelę do stanu instancji
                self.df_raw = temp_df

                # 4. Przekazujemy lokalną zmienną DataFrame do dalszego przetwarzania.
                df_hydra_queue = self._extract_hydra_queue(temp_df)
                df_smart_plan = self._extract_smart_plan(temp_df)
    ...
    def _extract_hydra_queue(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out = df.loc[:, [order_col, article_col, gp_col, side_col]].copy()
        assert isinstance(out, pd.DataFrame)

        # --- Nadpisz nazwy kolumn bezpośrednio (100% type-safe i szybsze) ---
        out.columns = ["order_id", "article", "grundprofil", "side"]
    ...
    # Wyciąga dane ze Smart Planu. Przy braku wymaganych kolumn zwraca None,
    # aby program działał dalej w trybie awaryjnym (fallback).
    def _extract_smart_plan(self, df: pd.DataFrame) -> pd.DataFrame | None:
        """Wyciąga metry, sztuki i status wykonania lub zwraca None."""
        try:
            needed_fixed = [
                "Stanowisko robocze",
                "Artykuł",
            ...
            # Używamy .loc i upewniamy linter, że to na 100% DataFrame
            df_tmp = df.loc[:, needed_fixed + zlecenie_cols].copy()
            ...
            out = df.loc[:, [*needed_fixed, order_col, side_col]].copy()
            assert isinstance(out, pd.DataFrame)
            ...
            for col in ("target_value_p", "target_value_s", "good_qty_p"):
                if col in out.columns:
                    numeric_values = out[col]
                    if not isinstance(numeric_values, pd.Series):
                        raise ValueError(f"Kolumna '{col}' występuje więcej niż raz.")
                    out[col] = (
                        numeric_values
                        .astype(str)
                    ...
                    temp_series = pd.Series(pd.to_numeric(out[col], errors="coerce"))
                    out[col] = temp_series.fillna(0.0)
            ...
            out["order_id"] = (
                out["order_id"].astype("string").str.strip().str.replace(r"\.0$", "", regex=True)
            )

            # Usuwamy zduplikowane kolumny po czyszczeniu ich nazw.
    ...
    # Pobiera dane z DB i generuje raport po otrzymaniu parametrów od użytkownika.
    def _finish_db_calculation(
        ...
        try:
            ...
            df_profiles = normalize_profiles_db_df(df_prof_raw)
            ...
            if df_orders is None or df_orders.empty:
                self.view.show_warning(
                    "Brak danych", "Baza SQL nie zwróciła żadnych zleceń dla tych maszyn."
                )
                return
            ...
            # Wyciągamy daty zakończenia poszczególnych maszyn z tekstu raportu.
            end_by_machine = {}
            ...
            # Jeśli zaznaczono "Zapisz terminy", zapisujemy snapshot na dysku.
            ...
            self.view.show_error(
                "Błąd obliczeń", f"Wystąpił problem podczas generowania raportu:\n{e}"
            )
            traceback.print_exc()

    # Generuje raport potwierdzenia zlecenia po wybraniu pliku i zlecenia.
    def handle_confirm_order(self):
        try:
            ...
            if df is None or df.empty:
                self.view.show_error(
                    "Brak danych", "Nie udało się wyciągnąć planu produkcji z wybranego pliku."
                )
                return
            ...
            if len(workplaces) != 1:
                self.view.show_error(
                    "Wybór maszyny", f"W danych wykryto wiele maszyn: {list(workplaces)}"
                )
                return
            ...
            if self.state.machine_cfg is None or self.state.machine_cfg.empty:
                df_cfg_mc, source, _missing = merge_db_and_csv_config(sync_missing_to_db=False)
            ...
            if df_mc is None or df_mc.empty:
                self.view.show_error(
                    "Brak konfiguracji", "Nie udało się wczytać konfiguracji maszyn."
                )
                return

            row = df_mc.loc[
                df_mc["workplace"].astype("string").str.strip() == str(workplace).strip()
            ]
            if row.empty:
                self.view.show_error(
                    "Brak konfiguracji", f"Nie znaleziono maszyny '{workplace}' w konfiguracji."
                )
                return
            ...
            if choice["mode"] == "speed":
                ...
                if abs(new_speed - default_speed) > 1e-9 and self.view.show_yes_no(
                    "Zapis do konfiguracji", "Zmieniono prędkość. Zapisać zmiany?"
                ):
                ...
                if new_pps != default_pps and self.view.show_yes_no(
                    "Zapis do konfiguracji", "Zmieniono szt./zmianę. Zapisać zmiany?"
                ):
            ...
            df_prof_raw = fetch_profiles_config()
            df_cfg = normalize_profiles_db_df(df_prof_raw)

            result_data = self._calculate_confirmation_result(
                df_cut, df_cfg, workplace, order_id, choice
            )
            ...
            # Program pokazuje okienko błędu zamiast nieoczekiwanie się zamykać.
    ...
    def _column_series(
        self, df: pd.DataFrame, column: str, default: float | str
    ) -> pd.Series:
        """Zwraca pojedynczą kolumnę albo serię z wartością domyślną.

        Duplikat nazwy kolumny zwracałby DataFrame, a nie Series, więc jest
        odrzucany przed operacjami takimi jak ``fillna``.
        """
        ...
        if isinstance(values, pd.DataFrame):
            raise ValueError(f"Kolumna '{column}' występuje więcej niż raz.")
        return values
    ...
    def _cut_from_order(self, df: pd.DataFrame, start_order_id: str) -> pd.DataFrame:
        ...
        result = df.iloc[hits[0] :].reset_index(drop=True)
    ...
    #     mask = tmp["order_id"].str.fullmatch(r"\d+").fillna(False)
    #     mask &= tmp["order_id"].str.len() < 12
    ...
    # --- Pomocnicze funkcje raportu SAP i odczytu informacji ze snapshota ---
    ...
    def _get_shift_info_from_snapshot(self, linia: str) -> str:
        ...
        try:
            ...
            if data.get("snapshot_date") != date.today().isoformat():
                ...
                return (
                    f"{date.today().strftime('%d.%m.%Y')} (zmiana 1) - "
                    f"stary zapis z {snapshot_date}"
                )
    ...
    # Sprawdza przy starcie, czy istnieje dzisiejszy snapshot, i ładuje go do stanu.
    def _load_snapshot_if_today(self) -> dict | None:
        # Ładuje snapshot tylko wtedy, gdy pochodzi on z dzisiejszego dnia.
        path = self._snapshot_path()
        if not path.exists():
            return None
    ...
    # --- Pomocnicze funkcje raportu potwierdzenia zlecenia ---
    def _cut_until_order(self, df: pd.DataFrame, order_id: str) -> pd.DataFrame:
        ...
        last_idx = hits[-1]
        result = df.iloc[: last_idx + 1].reset_index(drop=True)
        assert isinstance(result, pd.DataFrame)
        return result

    # Główny silnik kalkulacji dla potwierdzenia zlecenia.
    def _calculate_confirmation_result(
        self, dfx: pd.DataFrame, df_cfg: pd.DataFrame, workplace: str, order_id: str, choice: dict
    ) -> dict:
        ...
        df_cfg["setting_time"] = pd.to_numeric(
            self._column_series(df_cfg, "setting_time", 0.0), errors="coerce"
        )
        ...
        dfx = dfx.merge(
            df_cfg[["profile", "side", "setting_time"]], on=["profile", "side"], how="left"
        )

        # --- ZABEZPIECZENIE: Zamieniamy NaN na 0 przed wejściem do pętli! ---
        # Gwarantujemy linterowi, że po to_numeric to jest Series i używamy fillna
        setting_time_s = pd.Series(
            pd.to_numeric(self._column_series(dfx, "setting_time", 0.0), errors="coerce")
        )
        dfx["setting_time"] = setting_time_s.fillna(0).astype(int)
        dfx.loc[dfx["side"] == "0020", "setting_time"] = 0

        # Metry pozostałe (Opakowane w pd.Series, co gwarantuje linterowi poprawny typ)
        target_p = pd.Series(
            pd.to_numeric(self._column_series(dfx, "target_value_p", 0.0), errors="coerce")
        ).fillna(0.0)

        good_p = pd.Series(
            pd.to_numeric(self._column_series(dfx, "good_qty_p", 0.0), errors="coerce")
        ).fillna(0.0)

        unit = self._column_series(dfx, "unit_p", "M").astype("string").str.strip().str.upper()
        ...
        # Gwarantujemy linterowi, że wyciągnięta kolumna to Series, a nie Unknown
        safe_length_m = pd.Series(dfx["length_m"])
        total_m = float(safe_length_m.sum())

        # --- POPRAWNE ODLICZANIE SZTUK (Proporcja z metrów) ---
        target_s = pd.Series(
            pd.to_numeric(self._column_series(dfx, "target_value_s", 0.0), errors="coerce")
        ).fillna(0.0)

        # (target_p i good_p są już zadeklarowane i poprawnie otypowane wyżej,
        # więc nie trzeba ich tu nadpisywać. Możesz usunąć ich ponowną deklarację.)
        ...
        ratio.loc[mask_started] = (
            (target_p - good_p).clip(lower=0.0) / target_p.replace(0, 1)
        ).loc[mask_started]
        ...
        # Zbrojenia: unikalne pozycje z odrzuceniem pierwszego ustawienia.
        first_prof = dfx["profile"].iloc[0] if not dfx.empty else None
        first_side = dfx["side"].iloc[0] if not dfx.empty else None

        # 1. ZMIANA: Używamy krotki (tuple) z nawiasami okrągłymi zamiast listy [].
        # Type checkery wymagają tutaj typu niemutowalnego (Hashable).
        unique_setups = dfx[["profile", "side", "setting_time"]].drop_duplicates(
        ...
        # 2. ZMIANA: Zamiast .count() używamy wbudowanego w Pythona len().
        # Jest to szybsze, a linter zawsze wie, że len() zwraca bezpieczny int.
        setup_count = len(real_setups)

        # 3. ZMIANA: Linter zgubił typ kolumny po filtrowaniu.
        # Owijamy to w pd.Series, żeby zagwarantować mu, że to wciąż seria danych,
        # którą można bezpiecznie zsumować i zamienić na float.
        safe_setting_time = pd.Series(real_setups["setting_time"])
        total_setting_min = float(safe_setting_time.sum())
        ...
        return {
            ...
            "details": [
                ...
                (
                    "Start liczenia",
                    f"{pl_weekday_name(start_d)} ({start_d.isoformat()}) zmiana {start_shift}",
                ),
            ],
